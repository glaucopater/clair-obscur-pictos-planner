...
describe("usePictoLogic", () => {
  ...
  it("should filter pictos by attribute", () => {
    ...
    act(() => {
      ...
      result.current.setSelectedAttribute(["Health", "Defense"] as any);
    });

    expect(result.current.filteredPictos).toEqual([
      result.current.pictos[0], // Picto A (Health, Speed)
      result.current.pictos[1], // Picto B (Defense)
      result.current.pictos[3], // Picto D (Health)
      result.current.pictos[4], // Picto E (Defense)
    ]);
  });
  ...
  it.skip("should calculate total increased damage", () => {
    ...
    expect(result.current.totalEffectsSummary.totalBreakDamageIncrease).toBe(
      20
    );
  });
